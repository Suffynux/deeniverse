...
const Navbar = () => {
  ...
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [sidebarDropdownOpen, setSidebarDropdownOpen] = useState(false);
  // Separate state for the new dropdown in mobile (More Resources)
  const [sidebarMoreResourcesOpen, setSidebarMoreResourcesOpen] = useState(false);
  const [bannerVisible, setBannerVisible] = useState(false);
  const { scrollYProgress } = useScroll();
  ...
  const navItems = [
    ...
    { name: "Quizzes", path: "/quizzes" },
    { name: "Prayer Times", path: "/prayer-times" },
    // Removed "More Resources" from here to be converted to a dropdown
    // { name: "More Resources", path: "/more-resources" },
  ];
  ...
  return (
    <>
              ...
              <nav className="hidden md:flex items-center">
                ...
                <NavigationMenu viewport={false}>
                  <NavigationMenuList>
                    ...
                    {/* 👇 New More Resources dropdown implemented with shadcn menu */}
                    <NavigationMenuItem className="relative">
                      <NavigationMenuTrigger
                        className={`${navigationMenuTriggerStyle()} text-white bg-transparent !shadow-none focus:outline-none focus:ring-0`}
                      >
                      ...
                      <NavigationMenuContent className="!absolute left-0 top-full mt-2 !bg-transparent !shadow-none !ring-0 p-0 z-50">
                        <ul className="w-48 rounded-xl bg-gradient-to-br from-gray-800 to-gray-700 border border-gray-600 p-2 space-y-1 shadow-lg">
                          {moreResources.map((resource) => (
                            <li key={resource.id}>
                              <NavigationMenuLink asChild>
                                <Link
                                  // path is empty string for now, as requested
                                  to={resource.path} 
                                  className="block px-4 py-3 text-sm font-medium text-gray-200 rounded-lg
                                             hover:bg-amber-400 hover:text-gray-900 transition-all duration-200 ease-in-out
                                             hover:shadow-md hover:scale-[1.02] active:scale-[0.98] outline-none focus:outline-none focus:ring-0 bg-transparent"
                    ...
                    {/* 👆 End of new More Resources dropdown */}
      ...
      <div
        className={`${
          bannerVisible ? "pt-[128px] sm:pt-[120px]" : "pt-[64px]"
        }`}
      ></div>

      {/* Mobile Sidebar (updated for More Resources dropdown) */}
      <AnimatePresence>
        {sidebarOpen && (
          <>
            <motion.div
            ...
            >
              ...
              <nav className="flex flex-col p-4 space-y-3">
                ...
                {/* 👇 New More Resources Dropdown for Mobile */}
                <div>
                  <button
                    className="flex items-center gap-1 w-full text-lg font-medium text-white hover:text-amber-500 py-2 focus:outline-none transition-colors duration-200"
                    onClick={() => setSidebarMoreResourcesOpen((prev) => !prev)}
                  >
                    More Resources
                    <span className="ml-1">
                      {sidebarMoreResourcesOpen ? "▲" : "▼"}
                    </span>
                  </button>
                  <AnimatePresence>
                    {sidebarMoreResourcesOpen && (
                        ...
                        {moreResources.map((resource) => (
                          <li key={resource.id}>
                            <Link
                              ...
                              onClick={() => {
                                setSidebarOpen(false);
                                setSidebarMoreResourcesOpen(false); // Close this dropdown
                              }}
                            >
                              {resource.title}
                            </Link>
                ...
                {/* 👆 End of new More Resources Dropdown for Mobile */}
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="bg-amber-500 text-gray-900 font-semibold px-4 py-2 rounded-lg mt-4"
...
// The export below remains unchanged
export function ScrollProgressDemo() {
  return (
    <div className="z-10 rounded-lg p-4">
      <ScrollProgress className="top-[65px}" />
